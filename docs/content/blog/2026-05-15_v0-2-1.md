...
author: LHelge
created: 2026-05-15
description: A content-polish release. Wiki-links now accept `[[page#section]]` anchors, headings take `{#custom-id}` overrides, author config gains a dedicated `link` field, every page ships OpenGraph + Twitter card meta tags, and blog posts expose a reading-time estimate.
