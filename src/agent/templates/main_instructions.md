...
re-run the same `aphid agent` command to refresh them — new releases may introduce features
that the instructions need to teach agents about. Any local edits you have made will be
overwritten, so prefer adding project-specific guidance in a separate file.
